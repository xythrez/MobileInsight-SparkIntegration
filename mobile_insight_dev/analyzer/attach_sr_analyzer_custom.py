...
__all__ = ["CustomAttachSrAnalyzer"]
...
class CustomAttachSrAnalyzer(CustomKPIAnalyzer):

    def __init__(self):
        CustomKPIAnalyzer.__init__(self)
    ...
    def set_source(self,source):
        ...
        """
        CustomKPIAnalyzer.set_source(self,source)
        #enable LTE EMM logs
        source.enable_log("LTE_NAS_EMM_OTA_Incoming_Packet")
        source.enable_log("LTE_NAS_EMM_OTA_Outgoing_Packet")

    def clean_kpis(self):
        # Ensure the directory is removed if it exists, then recreate
        if os.path.exists('./kpis'):
            shutil.rmtree('./kpis')
        os.makedirs('./kpis')
